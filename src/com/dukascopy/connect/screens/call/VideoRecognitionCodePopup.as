package com.dukascopy.connect.screens.call {
	...
	public class VideoRecognitionCodePopup extends Sprite {
		...
		public const POPUP_CLOSE:Signal = new Signal("VideoRecognitionCodePopup.POPUP_CLOSE");
		
		public function VideoRecognitionCodePopup() {
			create();
		}
		...
		public function setSizes(screenWidth:int, screenHeight:int):void {
			...
			if (closeButton) {
				...
				closeButton.y = int(screenHeight - Config.DOUBLE_MARGIN - closeButton.height);
			}
		}
		...
		public function display(value:String):void {
			...
			text.bitmapData = TextUtils.createTextFieldData(value, screenHeight, 10, true, TextFormatAlign.CENTER, TextFieldAutoSize.LEFT, Config.FINGER_SIZE, true, 0xC5C8D8, 0x222533);
			text.x = int(screenWidth * .5 + text.width * 0.5);
			text.y = int(screenHeight * 0.5 - text.height * .5);
		}
