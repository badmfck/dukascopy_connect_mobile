package com.dukascopy.connect.screens.shop {
	...
	public class SubscriptionsScreen extends BaseScreen {
		...
		public function SubscriptionsScreen() { }
		...
		override public function initScreen(data:Object = null):void {
			...
			topBar.setData(Lang.mySubscriptions, true);
			_params.doDisposeAfterClose = true;
			list.setWidthAndHeight(_width, _height - topBar.trueHeight);
			
			Shop.S_ORDERS.add(updateList);
			setListData();
		}
		...
		private function setListData():void {
			...
			var subscriptions:Vector.<Order> = Shop.getOrders();
			
			list.setData(subscriptions, ListOrderItem, ["avatarURL"]);
		}
		
		private function onItemTap(data:Object, n:int):void {
			
			if (data == null || (data is Order) == false)
				return;
			
			if ((data as Order).product != null && (data as Order).product.productType.value == ProductType.TYPE_PAID_CHANNEL_SUBSCRIPTION)
			{
				if ((data as Order).product.targetData != null && (data as Order).product.targetData is ChatVO)
				{
					var chatVO:ChatVO = (data as Order).product.targetData as ChatVO;
