package com.dukascopy.connect.sys.photoGaleryManager.exif {
	...
	public class ExifLoader extends Loader {
		...
		private function onComplete(e:Event):void {
			...
			// Alexey added : pochemu nikto ne po4is4aet Listeneri? 
			_urlLoader.removeEventListener(Event.COMPLETE, onComplete);
			_urlLoader.removeEventListener(IOErrorEvent.IO_ERROR, forwardEvent);
			_urlLoader.removeEventListener(ProgressEvent.PROGRESS, forwardEvent);
			_urlLoader.removeEventListener(Event.OPEN, forwardEvent);
			_urlLoader.removeEventListener(HTTPStatusEvent.HTTP_STATUS, forwardEvent);
			_urlLoader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, forwardEvent);
			_urlLoader = null;
			// End Alexey added 
			dispatchEvent(e);
		}
