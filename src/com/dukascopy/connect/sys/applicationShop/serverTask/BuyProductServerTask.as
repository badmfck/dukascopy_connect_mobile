package com.dukascopy.connect.sys.applicationShop.serverTask
{
	 ...
	 */
	public class BuyProductServerTask extends BaseAction implements IServerTask
	{
		...
		public function BuyProductServerTask(product:ShopProduct, payWallet:String) {
		...
		public function execute():void {
			if (product != null) {
				if (product.productType.value == ProductType.TYPE_PAID_CHANNEL_SUBSCRIPTION)
				{
					...
					{
						PHP.requestBuyProduct(onRequestBuyResponse, (product.targetData as ChatVO).uid, product.count, product.cost.value, product.cost.currency);
					}
					else{
					...
					S_ACTION_FAIL.invoke("error");
					ApplicationErrors.add("wrong product type");
				}
			}
			else {
				S_ACTION_FAIL.invoke("error");
		...
		private function onRequestBuyResponse(respond:PHPRespond):void {
			if (respond.error == true) {
				...
				//TODO
				if (!isNaN(Number(respond.data))) {
					requestId = Number(respond.data);
					payForProduct();
				}
				else {
		...
		private function payForProduct():void {
			...
			requestPayData.description = product.getDescription();
			
			//TODO
			var details:Object;
		//	var details:Object = {payHash:banData.payHash, id:banData.id, type:"product"};
		...
		private function onBanPaidSuccess(transactionId:String):void {
			...
			requestFinishBuy(transactionId);
		}
		
		private function requestFinishBuy(transactionId:String):void {
			if (product.productType.value == ProductType.TYPE_PAID_CHANNEL_SUBSCRIPTION)
			{
				PHP.registerBuyProduct(onRequestFinishResponse, requestId, transactionId);
			}
			else{
				S_ACTION_FAIL.invoke("error");
				ApplicationErrors.add("wrong product type");
		...
		private function onRequestFinishResponse(respond:PHPRespond):void {
			if (respond.error == true) {
				var message:String = ErrorLocalizer.getText(respond.errorMsg);
				S_ACTION_FAIL.invoke(message);
			}
				...
				S_ACTION_SUCCESS.invoke(product);
			}
			else { 
				S_ACTION_FAIL.invoke(Lang.serverError);
				ApplicationErrors.add("bad server data");
