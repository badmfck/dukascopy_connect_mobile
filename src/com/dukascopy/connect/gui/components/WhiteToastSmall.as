package com.dukascopy.connect.gui.components 
{
	 ...
	 */
	public class WhiteToastSmall extends Sprite
	{
		...
		public function WhiteToastSmall(text:String, itemWidth:int, itemHeight:int, completeHandler:Function, time:Number) 
		{
			this.time = time;
		...
		//	addChild(back);
			...
			textImage.bitmapData = TextUtils.createTextFieldData(text, itemWidth - Config.DIALOG_MARGIN * 2 - Config.FINGER_SIZE * 2, 10, 
																...
																FontSize.BODY, true, Color.WHITE, Style.color(Style.TOP_BAR_TEXT_COLOR));
			
			white.graphics.beginFill(Style.color(Style.TOP_BAR_TEXT_COLOR));
			white.graphics.drawRoundRect(0, 0, textImage.width + Config.DIALOG_MARGIN * 2, textImage.height + Config.FINGER_SIZE * 0.4, Config.FINGER_SIZE*.3, Config.FINGER_SIZE*.3);
			white.graphics.endFill();
			
			maskClip.graphics.beginFill(0xFFFFFF);
			maskClip.graphics.drawRect(0, 0, textImage.width + Config.DIALOG_MARGIN * 2, textImage.height + Config.FINGER_SIZE * 0.4);
