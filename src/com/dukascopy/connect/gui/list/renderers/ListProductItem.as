package com.dukascopy.connect.gui.list.renderers {
	...
	public class ListProductItem extends BaseRenderer implements IListRenderer {
		...
		private var tfOrderEarnings:TextField;
		
		public function ListProductItem() {
			headerHeight = Config.FINGER_SIZE*.4;
				bg = new Shape();
			...
			tfOrderEarnings = new TextField();
				format4.size = Config.FINGER_SIZE * .24;
				format4.align = TextFormatAlign.LEFT;
				format4.color = 0x808080;// 0x363E4E;
				tfOrderEarnings.defaultTextFormat = format4;
				tfOrderEarnings.wordWrap = false;
				tfOrderEarnings.multiline = false;
				tfOrderEarnings.text = "|";
				tfOrderEarnings.height = tfOrderEarnings.textHeight + 4;
				tfOrderEarnings.text = "";
			addChild(tfOrderEarnings);
		...
		public function getHeight(data:ListItem, width:int):int {
			if (!(data.data is ShopProduct))
				return Config.FINGER_SIZE_DOT_75;
			return Config.FINGER_SIZE*1.75;
		}
		
		private function setText(cVO:ChatVO, w:int, earnings:String):int {
			...
			tfOrderEarnings.text = earnings;
			tfOrderEarnings.width = tfOrderEarnings.textWidth + 4;
			tfOrderEarnings.x = w - tfOrderEarnings.width - Config.MARGIN;
			tfOrderEarnings.y = int(Config.FINGER_SIZE * .05);
		...
		public function getView(li:ListItem, h:int, w:int, highlight:Boolean = false):IBitmapDrawable {
			if (li.data == null || li.data is ShopProduct == false) {
		...
		//	bg.height = h;
			...
			var product:ShopProduct = li.data as ShopProduct;
			var cVO:ChatVO = product.targetData as ChatVO;
			...
			setText(cVO, w, Lang.totalEarnings + ": " + product.getEarnings());
		...
		public function dispose():void {
			...
			if (tfTitle != null)
				tfTitle.text = "";
			tfTitle = null;
			if (tfOrderEarnings != null)
				tfOrderEarnings.text = "";
			tfOrderEarnings = null;
			if (tfLastMessage != null)
