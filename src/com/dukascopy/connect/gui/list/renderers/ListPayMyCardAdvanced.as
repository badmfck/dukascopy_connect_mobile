package com.dukascopy.connect.gui.list.renderers {
	 ...
	 * @author IgorBloom
	 */
	public class ListPayMyCardAdvanced extends BaseRenderer implements IListPayCardAdvanced {
...
//		private var dcLogoCanvas:Bitmap;
		...
		public function ListPayMyCardAdvanced() {
			roundBg = new PatternShapeBox(BMD_LIGHT, width, itemHeight);
			...
			//logoCanvas.bitmapData = Assets.getAsset(Assets.PAYMENT_ICON_MASTERCARD);//PAYMENT_ICON_VISA
...
//			UI.disposeBMD(dcLogoCanvas.bitmapData);
			...
			var balance:String = data.available ? data.available : "";
...
//			var logoAsset:DisplayObject = cardType == "virtual card" ? getBlackLogoAsset() : getWhiteLogoAsset();
			...
			//var str:String = CardCommon.getTypeCardByNumberCard(data.number);
			...
			cardTitleCanvas.bitmapData = UI.renderTextShadowed(data.status /*+ " " + cardType*/, width - (padding + innerPadding) * 2, fontSizeX, false, false, TextFormatAlign.LEFT, TextFieldAutoSize.LEFT, fontSizeX, false, FONT_SHADOW_COLOR, FONT_SHADOW_COLOR, FONT_COLOR, true);

			// Valid until 
			cardValidTitleCanvas.bitmapData = UI.renderTextShadowed(/*"VALID\nTHRU"*/ "", (padding + innerPadding) * 2, fontSizeX * .45, false, false, TextFormatAlign.LEFT, TextFieldAutoSize.LEFT, fontSizeX * .45, false, FONT_SHADOW_COLOR, FONT_SHADOW_COLOR, FONT_COLOR, true);
		...
		/*	if (cardType == "virtual card") {
			...
			roundBg.bitmapData = /*cardType == "virtual card" ? */BMD_LIGHT /*: BMD*/;

			// overlay 
			bgOverlay.x = padding;
			bgOverlay.width = width - padding * 2;
			bgOverlay.height = itemHeight - padding;
			bgOverlay.visible = /*cardType == "virtual card" ?*/ true /*: false*/;

			// hit zones
			var offsetBounds:int = 20; // to make hitzone bigger for tap

			var hitZones:Array = [{
				type: HitZoneType.CVV,
				x: cvvCanvas.x - offsetBounds,
				y: cvvCanvas.y - offsetBounds,
				width: cvvCanvas.width + offsetBounds * 2,
				height: cvvCanvas.height + offsetBounds * 2
			}];
...
//			var logoAsset:DisplayObject = cardType == "virtual card" ? getBlackLogoAsset() : getWhiteLogoAsset();
			...
			UI.disposeBMD(logoCanvas.bitmapData);
//			UI.disposeBMD(dcLogoCanvas.bitmapData);
			logoCanvas.bitmapData = CardCommon.getBitmapDataCardByNumberCard(data.number);
			//trace("CARD TYPE ===>>>>"+cardType);
			...
			cardTitleCanvas.bitmapData = UI.renderTextShadowed(data.status /*+ " " + cardType*/, width - ( innerPadding) * 2, fontSizeX, false, false, TextFormatAlign.LEFT, TextFieldAutoSize.LEFT, fontSizeX, false, FONT_SHADOW_COLOR, FONT_SHADOW_COLOR, FONT_COLOR, true);

			// Valid until 
			cardValidTitleCanvas.bitmapData = UI.renderTextShadowed(/*"VALID\nTHRU"*/"", ( innerPadding) * 2, fontSizeX * .45, false, false, TextFormatAlign.LEFT, TextFieldAutoSize.LEFT, fontSizeX * .45, false, FONT_SHADOW_COLOR, FONT_SHADOW_COLOR, FONT_COLOR, true);
