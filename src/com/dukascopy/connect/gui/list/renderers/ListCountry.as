package com.dukascopy.connect.gui.list.renderers{
	 ...
	 * @author IgorBloom
	 */
	public class ListCountry extends BaseRenderer implements IListRenderer{
		...
		public function ListCountry() {
			...
			tfCode.defaultTextFormat = format;
			tfCode.width = Config.FINGER_SIZE;
			tfCode.textColor = Style.color(Style.COLOR_TEXT);
		...
		public function getView(li:ListItem,h:int, width:int,highlight:Boolean=false):IBitmapDrawable {
			...
			if (data.length == 2) {
				...
				tfCountry.width = width - tfCountry.x - padding - tfCode.width;
			}
			
			tfCode.x =  tfCountry.x + tfCountry.width;
			
			tfCountry.y = Math.round((h - tfCountry.textHeight) * .5);
			tfCode.y = Math.round((h-tfCode.textHeight) * .5);
