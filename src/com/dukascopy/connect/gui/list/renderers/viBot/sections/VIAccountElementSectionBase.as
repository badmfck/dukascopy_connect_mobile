package com.dukascopy.connect.gui.list.renderers.viBot.sections {
	...
	public class VIAccountElementSectionBase extends Sprite {
		...
		static public const CORNER_RADIUS:int = Config.FINGER_SIZE * .2;
		static public const CORNER_RADIUS_DOUBLE:int = CORNER_RADIUS * 2;
		public const BIRD_SIZE:int = Config.FINGER_SIZE * .3;
		...
		public function VIAccountElementSectionBase() {
			bg = new Shape();
			addChild(bg);
			...
			lineHeight = int(Config.FINGER_SIZE * .02);
			if (lineHeight == 0)
				lineHeight = 1;
		...
		protected function drawBG():void {
			...
			var cr:int = Config.FINGER_SIZE * .3;
			
			var cornerAnchorY1:int = trueHeight - cr * .5;
			var cornerAnchorY2:int = trueHeight - cr * .1;
			var birdY:int = trueHeight - cr * .3;
			...
			if (isFirst == true) {
				...
				bgGfx.curveTo(right, 0, right, cr);
			} else {
			...
			if (isLast == true) {
				...
				if (isMine == true) {
					bgGfx.lineTo(right, bottomCornerY - Config.FINGER_SIZE * .15);
					bgGfx.curveTo(right, cornerAnchorY1, trueWidth, cornerAnchorY1);
					bgGfx.curveTo(right + BIRD_SIZE * .35, cornerAnchorY2, right - cr * .2, birdY);
					bgGfx.curveTo(LEFT_WITH_CORNER + widthWithoutCorners + cr * .5, trueHeight, LEFT_WITH_CORNER + widthWithoutCorners, trueHeight);
				} else {
				...
				if (isMine == true) {
					...
					bgGfx.curveTo(BIRD_SIZE + cr * .5, trueHeight, BIRD_SIZE + cr * .2, birdY);
					bgGfx.curveTo(BIRD_SIZE * .65, cornerAnchorY2, 0 , cornerAnchorY1);
					bgGfx.curveTo(BIRD_SIZE, cornerAnchorY1, BIRD_SIZE, bottomCornerY - BIRD_SIZE * .7);
				}
			} else {
