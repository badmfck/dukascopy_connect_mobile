package com.dukascopy.connect.gui.list.renderers.chatMessageElements 
{
	 ...
	 */
	public class VoiceMessageRenderer extends Sprite
	{
		...
		public function VoiceMessageRenderer() 
		{
			initTextFormats();
			create();
		}
		...
		private function create():void 
		{
			...
			progressMask.graphics.drawRoundRect(0, 0, 
		...
		{
			return int(buttonSize + Config.MARGIN * 2 + Config.DOUBLE_MARGIN);
		}
		...
		public function draw(data:VoiceMessageVO, maxWidth:int):void
		{
			iconPlay.visible = false;
			iconStop.visible = false;
		...
		//	progressClip.visible = data.isPlaying;
			
			if (data.currentTime == data.duration || (data.currentTime == 0 && !data.isPlaying))
			{
		...
		public function destroy():void
		{
			...
			displayItems["back"] = back;
			displayItems["message"] = message;
			displayItems["iconPlay"] = iconPlay;
			displayItems["iconStop"] = iconStop;
			displayItems["iconLoading"] = iconLoading;
			displayItems["iconLoading"] = progressMask;
			displayItems["iconLoading"] = progressClip;
			
			destroyItems(displayItems);
			displayItems = null;
			
			textFormatTime = null;
		}
		
		protected function destroyItems(displayItems:Dictionary):void {
			var index:int;
			for (var key:String in displayItems) {
				if (displayItems[key]) {
					UI.destroy(displayItems[key]);
					if (displayItems[key] is DisplayObject) {
						this[key] = null;
					}
				}
				delete displayItems[key];
			}
		}
		
		public function getHitZones():Array 
		{
			var playPoint:Point = parent.globalToLocal(localToGlobal(new Point(iconPlay.x - Config.MARGIN, iconPlay.y - Config.MARGIN)));
			var switchPoint:Point = parent.globalToLocal(localToGlobal(new Point(iconLoudSpeaker.x - Config.MARGIN, iconLoudSpeaker.y - Config.MARGIN)));
			
			playPoint.x = playPoint.x;
			switchPoint.x = switchPoint.x;
