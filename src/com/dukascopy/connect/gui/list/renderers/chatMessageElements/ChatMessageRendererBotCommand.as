package com.dukascopy.connect.gui.list.renderers.chatMessageElements {
	...
	public class ChatMessageRendererBotCommand extends ChatMessageRendererBase implements IMessageRenderer {
		...
		public function ChatMessageRendererBotCommand() {
			super();
		...
		protected function getMegaTextHeightByChatMessage(messageVO:ChatMessageVO/*, isForwarded:Boolean*/,  targetWidth:int):int {
			...
			var res:int = megaText.setText(targetWidth, txt, colorText, textSize, "#" + ct.color, 1.5, messageVO.wasSmile);
		...
		public function draw(messageVO:ChatMessageVO, maxWidth:int, listItem:ListItem = null, securityKey:Array = null, minWidth:int = -1):void {
			...
			if (isLastMessageForwarded == true) {
				if (isMine == true) {				
					forwardView.setQuotesColor(AppTheme.GREY_SEMI_LIGHT);
					forwardView.setTextColor(AppTheme.WHITE);
				} else {
					forwardView.setTextColor(AppTheme.GREY_MEDIUM);
					forwardView.setQuotesColor(AppTheme.GREY_MEDIUM);
				}
				...
				megaText.x = hTextMargin;
				megaText.y = vTextMargin;
				bgW = megaText.tfTextWidth + hTextMargin * 2;
				removeForwardView();
			}
