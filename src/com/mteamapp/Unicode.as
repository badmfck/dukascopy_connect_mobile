...
package com.mteamapp
{
	...
	public class Unicode
	{
		...
		public static var adad:String = '٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹۰۱۲۳۴۵۶۷۸۹����٪٪';
		...
		public function fastUnicodeOnLines(yourTextField:TextField,tex:String,detectLanguage:Boolean = true):void
		{
			...
			for(var j:int =0 ;j<parag.length;j++){
				...
				while((lastNumLines = yourTextField.numLines)>1 && cnt<1000){
					...
					yourTextField.text = spaces + parag[j];
					lastNumLines = yourTextField.numLines;
					var cashedText:String = yourTextField.getLineText(lastNumLines - 1);
					var lineIndex:Number = yourTextField.getLineOffset(lastNumLines - 1);
					
					var indexOfSplitters:Number = Infinity;
					for(i=0;i<splitters.length;i++){
						var J:int = cashedText.indexOf(splitters[i]);
						if(J!=-1){
		...
		public function HTMLfastUnicodeOnLines(yourTextField:TextField,tex:String,justify:Boolean = true):void
		{
			//trace("tex : "+tex);
			var myTextcash:Array = entersCorrection(tex).split(String.fromCharCode(10));
			var parag:Array = [];
		...
		public function toUnicode(ch:String,style:Number=0):String{
			if(ch=='')
			{
				return '' ;
			}
		...
		private function MESfindeType(ch:String):int
		{
		...
		private function MESbekesh(character,no:int = -1):String{
		...
		public function getPorp(htm:String,color:Boolean=false,size:Boolean=false,align:Boolean=false,lending:Boolean=false):String{
			var cash:String = ''
			if(color){
				cash = "COLOR=\"";
			}else if(size){
			...
			var htm2:String = htm.substring(I)
			var num:String = htm2.substring(0, htm2.indexOf('\"'));
			return num;
		}
		
		
		
		public function setPorp(htm,variable,color=false,size=false,align=false,lending=false):String{
			var cash:String = ''
			var htm2:String=''
			if(color){
				cash = "COLOR=\"";
		...
		public function justHTML(field_txt,tex:String,autoSize_F:Boolean=true):void{
			field_txt.text = 's'
			var cashAlign:String = getPorp(field_txt.htmlText,false,false,true)
			if(autoSize_F){
		...
		private function getLastSplit(tex):Number{
			var mem:Number = 0;
			var cash:String;
		...
		private function justifyUnicode(field_txt:TextField, matn:String):String{
			field_txt.text = '.';
		...
		private function htmlSplit(tex,I):Array{
			var matn1:String = '';
			var matn2:String = '';
